...
public class ClientGlobal {
    ...
    public static void init(FastDfsConfig config) throws MyException {
        String[] szTrackerServers;
        String[] parts;
        ...
        if (config.getTrackerServer() == null || config.getTrackerServer().trim().isEmpty()) {
            throw new MyException("item \"tracker_server\" in not found");
        }
        szTrackerServers = config.getTrackerServer().split(",");
        ...
        g_connection_pool_enabled = config.isConnectionPoolEnabled();
        g_connection_pool_max_count_per_entry = config.getConnectionPoolMaxCountPerEntry();
        g_connection_pool_max_idle_time = config.getConnectionPoolMaxIdleTime();
        if (g_connection_pool_max_idle_time < 0) {
            g_connection_pool_max_idle_time = DEFAULT_CONNECTION_POOL_MAX_IDLE_TIME;
        }
        g_connection_pool_max_idle_time *= 1000;
        g_connection_pool_max_wait_time_in_ms = config.getConnectionPoolMaxWaitTimeInMs();
        if (g_connection_pool_max_wait_time_in_ms < 0) {
            g_connection_pool_max_wait_time_in_ms = DEFAULT_CONNECTION_POOL_MAX_WAIT_TIME_IN_MS;
        }
    }
